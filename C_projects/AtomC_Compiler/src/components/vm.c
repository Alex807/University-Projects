...
void vmInit() {
	...
	addFnParam(fn,"i",(Type){TB_INT,NULL,-1}); //for int code VM

	Symbol *put_d_fn = addExtFn("put_f", put_f,(Type){TB_VOID,NULL,-1}); 
	addFnParam(put_d_fn,"n",(Type){TB_DOUBLE,NULL,-1}); //for double code VM
}
	...
	for(;;){
		...
		switch(IP->op){
			...
			case OP_ADD_F: 
				...
				dBefore = popf();
				pushf(dTop+dBefore);
				printf("ADD.f\t// %g+%g -> %g", dBefore, dTop, dTop+dBefore);
				IP=IP->next;
				break;
			case OP_LESS_I:
				iTop=popi();
			...
			case OP_LESS_F: 
				dTop = popf();
				dBefore = popf();
				int result = (dBefore < dTop) ? 1 : 0;  // explicitly create boolean result
				pushi(result);                          // push as integer for JF to use
				printf("LESS.f\t// %g<%g -> %d", dBefore, dTop, result);
				IP = IP->next;
				break;
...
Instr *genTestProgram_Double(){
    Instr *code=NULL;
    // Push argument 2.0 and call f
    addInstrWithDouble(&code,OP_PUSH_F, 2.0);
    Instr *callPos=addInstr(&code, OP_CALL);
    addInstr(&code,OP_HALT);

    // Function f code starts here
    callPos->arg.instr=addInstrWithInt(&code, OP_ENTER , 1); //use this line to put in stack a counter of local variables(NOT a type)
    ...
    addInstr(&code, OP_LESS_F);                           // compare i < n
    Instr *jfAfter = addInstr(&code,OP_JF);

    // put_f(i);
    addInstrWithInt(&code,OP_FPLOAD,1);                  // load i for put_f
    ...
    addInstr(&code, OP_ADD_F);                            // add them
    addInstrWithInt(&code, OP_FPSTORE, 1);                 // store back to i

    // } (jump back for next iteration)
    addInstr(&code,OP_JMP)->arg.instr=whilePos;

    // return from function
    jfAfter->arg.instr=addInstrWithInt(&code, OP_RET_VOID, 1); //return type is VOID(as the proccessed code)
