...
int size = MATRIX_SIZE; //use as global_var to can write less latters in indexing a matrix element in each source_file
...
#pragma omp parallel num_threads(nr_of_threads), default(none), private(i, j, k, temp), shared(A, B, parallel_result, chunk, size)  
    { 
...
#pragma omp parallel num_threads(nr_of_threads), default(none), private(i, j, k, aik), shared(A, B, parallel_result, chunk, size)
    {
#pragma omp for schedule(static, chunk) collapse(2) 
        for (i = 0; i < MATRIX_SIZE; i++) {
            for (k = 0; k < MATRIX_SIZE; k++) {
...
void matrix_multiplication_parallel_V3(int nr_of_threads, int chunk) { 
    ...
    #pragma omp parallel num_threads(nr_of_threads), default(none), private(i, j, k, sum), shared(A, B, parallel_result, chunk, size)
    {
        #pragma omp for schedule(static, chunk) collapse(2)
        for (j = 0; j < MATRIX_SIZE; j++) {
            for (i = 0; i < MATRIX_SIZE; i++) {
...
void matrix_multiplication_parallel_V4(int nr_of_threads, int chunk) { 
    ...
    #pragma omp parallel num_threads(nr_of_threads), default(none), private(i, j, k, bjk), shared(A, B, parallel_result, chunk, size)
    {
        #pragma omp for schedule(static, chunk) collapse(2) 
        for (j = 0; j < MATRIX_SIZE; j++) {
            for (k = 0; k < MATRIX_SIZE; k++) {
